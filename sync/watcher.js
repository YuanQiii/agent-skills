...
const fs = require('fs');
const { config, log, retry, getTimestamp, parseSkillMd } = require('./utils');
const { generateCatalog } = require('./catalog');
const { translateAndSave, loadTranslations } = require('./translator');
...
async function translateNewSkillIfNeeded(filePath) {
  ...
  if (!parsed || !parsed.desc) return null;

  log(`🔤 翻译新技能 [${skillName}]: ${parsed.desc.substring(0, 30)}...`);
  const cn = await translateAndSave(skillName, parsed.desc);
  log(`✅ 翻译完成: ${cn}`);
...
module.exports = { startWatcher, syncToGitHub };
